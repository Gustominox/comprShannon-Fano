...
int do_freqAUX(unsigned char *buffer_e, unsigned char *buffer_s, unsigned long block_size){
...
void do_freqM (char *nome_do_fich, long long *n_blocks, unsigned long *size_of_last_block, unsigned long *block_size){
    ...
    for (int j = 0; j < *n_blocks-1; j++){
        ...
        unsigned long tmh_buffer_s = do_freqAUX(buffer_e, buffer_s, *block_size);
        
        fwrite(buffer_s, tmh_buffer_s, 1 , f_s);
    }
    fread(buffer_e, *size_of_last_block, 1, f_e);
    unsigned long tmh_buffer_s = do_freqAUX(buffer_e, buffer_s, *size_of_last_block);
